...
void main() {
  ...
  final projectName = pubspec.firstWhere((l) => l.startsWith('name:')).split(':').last.trim();
  final projectVersion = pubspec.firstWhere((l) => l.startsWith('version:')).split(':').last.trim();
  final projectDesc = pubspec.firstWhere((l) => l.startsWith('description:')).split(':').last.trim();
  ...
  buffer.writeln('This section is dynamically generated from the `lib/src/features/` layer.');
  buffer.writeln('');
  ...
  if (featuresDir.existsSync()) {
    ...
    for (final feature in features) {
      ...
      if (readme.existsSync()) {
        desc = readme.readAsLinesSync().firstWhere((l) => l.isNotEmpty && !l.startsWith('#'), orElse: () => desc);
      }
      ...
      final layers = feature.listSync().whereType<Directory>().map((d) => d.path.split('/').last.toUpperCase()).toList();
  ...
  final devDepsIndex = pubspec.indexWhere((l) => l.trim() == 'dev_dependencies:');
  
  if (depsIndex != -1) {
    final deps = pubspec.sublist(depsIndex + 1, devDepsIndex != -1 ? devDepsIndex : null)
        .where((l) => l.trim().isNotEmpty && l.contains(':') && !l.trim().startsWith('#') && !l.trim().startsWith('sdk:'))
    ...
    for (final dep in deps) {
      ...
      if (dep == 'cryptography' || dep == 'encrypt') usage = 'Industrial security.';
  ...
  buffer.writeln('');
  buffer.writeln('1. [Vault Cryptography](.agent/skills/vault_cryptography/SKILL.md)');
  buffer.writeln('2. [UI/UX Pro Max](.agent/skills/ui-ux-pro-max/SKILL.md)');
  buffer.writeln('3. [Senior Flutter Architect](.agent/skills/senior-flutter-architect/SKILL.md)');
  buffer.writeln('4. [Secure Storage Patterns](.agent/skills/secure-storage-patterns/SKILL.md)');
  buffer.writeln('');

  buffer.writeln('---');
  buffer.writeln('*Status: **High-Fidelity Automated Output**. Date: ${DateTime.now().toLocal()}*');
